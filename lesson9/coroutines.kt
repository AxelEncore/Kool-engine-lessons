...
// Если все эти процессы делать через onUpdate и таймеры вручную - это быстро превращается в кашу

// Корутины решают эту проблему:
// 1. позволяют писать время как обычный код: подождал -> сделал действие -> подождал -> действие
// 2. в процессе выполнения н замораживает игру и UI
// 3. удобно отменяются (например яд перезапускается, если наложили новый, а старый яд отменяем)
...
// cancel() - становить выполнение корутины (например снять эффект яда)
...
// Это просто безопаснее чем глобальные корутны, про которые мы можем забыть или не сохранить
...
class EffectManager(
    ...
    // scope - область где и будут запускаться и жить корутины
    ...
    fun applyPoison(ticks: Int, damagePerTick: Int, intervalMs: Long){
        poisonJob?.cancel()
        // если яд, уже был применен, аннулируем его (отменяем)
                ...
                Row {
                    ...
                    Button("Атаковать (кулдаун 1200мс)"){
                        modifier.margin(end = 8.dp).onClick{
                            ...
                            if (!cd.canAttack()){
                                pushLog(game, "Атаковать нельзя: кульдаун еще идет")
                                return@onClick
                            }
