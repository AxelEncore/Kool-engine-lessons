...
class AlchemistQuest: QuestDefinition{
    ...
    override fun stateDescription(stateName: String): String {
        return when(safeState(stateName)){
            AlchemistState.START -> "Поговорить с Алхимиром"
...
class GuardQuest: QuestDefinition{
    ...
    // перезапишите функцию для описания состояний
...
class QuestManager(
    ...
    // Получить состояние квеста нынешнее
    // И то к которому он перейдет
    ...
    // Публикуете событие, что прогресс игрока сохранен
    ...
    private fun handleEvent(event: GameEvent) {
        ...
        for (quest in quests) {
            ...
            if (next != current) {
                ...
                // Любое изменение квеста = контрольная точка сохранения
                bus.publish(
                    PlayerProgressSaved(
                        playerId = pid,
                        reason = "Quest ${quest.questId} изменен в сосотяние $next"
                    )
                )
            }
        }
